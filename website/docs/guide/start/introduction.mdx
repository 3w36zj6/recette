...
```ts
cli.command("deploy [env] --force|-f --timeout=<number> [...services]", (c) => {
  const env = c.arg("env"); // string (required positional argument)
  const force = c.flag("force"); // boolean (flag)
  const timeout = c.option("timeout"); // number | undefined (option with value)
  const services = c.args("services"); // string[] (variadic arguments)
});
```
